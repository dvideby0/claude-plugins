/**
 * Reports are views, not the source of truth. Everything here is a query
 * against the store and can be regenerated at any time.
 *
 * Nothing is written to disk. Files in the repository were how this was read
 * before there was an app to read it in; now the same content is returned to
 * whoever asked and rendered there, so nothing goes stale in a working tree
 * or needs to be gitignored.
 */
...
function auditReport(db: Db, tasks: Task[]): string {
  ...
  for (const severity of ["critical", "high", "medium", "low"]) {
    ...
    if (group.length > 50) {
      lines.push(`- … and ${group.length - 50} more in the full task list this report returned`);
    }
    lines.push("");
  }
...
export function buildReports(db: Db): ExportResult {
