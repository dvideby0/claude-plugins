...
**Task-context query** (QUERY-001). `brief` accepts a task, optional targets,
...
**Measured retrieval ranking** (QUERY-001). All reviewed evidence is retrieved in
...
**Change-aware retrieval** (QUERY-001). A bounded async Rust adapter consumes
...
rejects escaping and non-UTF-8 paths, and bounds status to 30 s, 2 MiB and 256
normalized changes. The planner preserves explicit targets ahead of unrelated
dirty files and treats pre-refresh deletes and renames as stale historical
evidence. A clean-baseline fixture scores 1.0 recall and coverage with zero
irrelevant paths.
...
`tsconfig.json` into the workspace.
